		...
		for(var i in this.model.users.models) {
			if(this.model.users.models[i].attributes.name == chat.attributes.name) {
				if(this.model.users.models[i].attributes.isStaff) {
					...
					icon = ' <img class="modicon" src="http://vignette2.wikia.nocookie.net/clubpenguin/images/c/c7/Moderator_badge.png/revision/latest?cb=20120723155212">';
				}
				break;
			}
		}
			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus) { //Only annoy people if the window isn't focused
						document.getElementById('sound').innerHTML = '<audio src="http://vignette2.wikia.nocookie.net/clubpenguin/images/c/ca/Ding_Sound_Effect.ogg/revision/latest?cb=20160117104141" autoplay=""></audio>';
